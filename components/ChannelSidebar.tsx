...
import { Hash, Plus, Copy, Check, MessageSquare, UserPlus } from 'lucide-react'
...
export default function ChannelSidebar({ profile }: { profile: Profile }) {
  ...
  const [dms, setDms]                   = useState<(DirectMessage & { otherUser: Profile; lastMsg?: string })[]>([])
    ...
    return (
      <div className="w-60 bg-[#2b2d31] flex flex-col shrink-0">
        ...
        <div className="px-2 pt-4 pb-1">
          <p className="text-xs font-semibold uppercase text-[#949ba4] px-2 mb-2 tracking-wide">
            Direct Messages
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2">
          ...
          {dms.length === 0 && (
            <p className="text-xs text-[#4e5058] px-2 mt-2">
