...
export default function GroupArea({ group: initialGroup, initialMessages, initialMembers, currentUserId }: Props) {
  ...
  const currentMyProfile = members.find(m => m.user_id === currentUserId)?.profiles
  const currentUserUsername = currentMyProfile?.username

  const mentionableProfiles = members.map(m => m.profiles).filter((p): p is Profile => !!p)
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={msg.id}>
                ...
                <div className={`flex items-start gap-4 px-2 py-0.5 rounded hover:bg-[var(--theme-message-hover)] group ${!grouped ? 'mt-4' : ''} ${isHighlighted ? 'bg-[#f0b132]/20' : ''}`}>
                  {!grouped ? (
                    <div
